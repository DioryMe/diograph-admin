...
export class DioryForm extends React.Component<DioryFormProps, undefined> {
  ...
  render() {
    return <div>
      Name: <input value={this.props.diory.name || ""} onChange={(event) => { this.props.onDioryChange({name: event.target.value}) }}  />
      ...
      Gps: <input value={this.props.diory.geo.longitude || ""} onChange={(event) => { this.props.onDioryChange({geo: {longitude: event.target.value}}) }}  />
